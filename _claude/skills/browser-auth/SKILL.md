...
- **フォーカスを奪わない（既定）。** `drive.py` は `goto` の完了を待たずに元のアプリへ
  フォーカスを1回返す。ユーザーが入力する場面だけ `--focus` を付ける。
  ⚠️ **完全には防げていない。** 単発の `activate` のあと Chrome が起動完了時に
  前面を取り返すことがある（実測: `now: Google Chrome`）。
  🔴 **`osascript` を繰り返し呼んで張り付く実装にしてはいけない。** 一度そうしたところ、
  `osascript` 1回あたり最大5秒のタイムアウトが積み上がり、**起動に3分以上かかった**
  （2026-09-09 実測、実装は撤去済み）。フォーカス制御のために起動を壊さない。
  次に試すなら、起動直後に CDP `Browser.setWindowBounds` の `windowState: "minimized"` で
  最小化し、入力時だけ Dock から戻してもらう形（最小化状態でスクリーンショットが
  撮れるかは未確認）。
